  ...
  describe("Calculating a cell's future state", () => {
    it("Should have a willBeAlive function. @willBeAlive-function", () => {
      ...
      assert.equal(
        nextGen.length,
        6,
        "1. Have you created a 'willBeAlive' function that calculates if a cell will be alive in the next game state?"
      );
      assert(
        containsAll(
          [
        ...
        "2. Have you created a 'willBeAlive' function that calculates if a cell will be alive in the next game state?"
      );
    });
  });
